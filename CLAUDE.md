...
- **Dead stock** (`mercancia_quieta(dias)`, migration 35, shown in Reportes): products that still have stock but have not sold in that window, and how much money they tie up at cost. Admin-only, because the value is computed at cost. It returns a **jsonb object**, not rows: totals are summed in Postgres over the whole catalogue and only the 100 highest-value products travel, so the 1,000-row cap can never make the total silently short. It ignores the page's date range on purpose — the question is always "as of today, what has not moved".
