...
- **Costs are business-sensitive.** Sellers must not read them, and hiding them in the UI is not enough: with their own session they can query Supabase directly. `entradas`/`ajustes`/`detalle_devoluciones` are admin-only, `devoluciones` has no cost column, and `producto_historial` text never includes prices (migration 27). Sellers get returned quantities through `devuelto_por_linea()`. **Still open (phase 2):** `productos.costo` and `detalle_ventas.costo_unitario` are readable by the whole business, because `registrar_venta` and the inventory trigger run as the seller and need them; closing that means moving cost to its own admin-only table and making those functions `SECURITY DEFINER`.
